...
public class LoginTest extends AbstractTestNGSpringContextTests {
    ...
    @Test(groups = {  "missingPassword" })
    public void verifyLoginmissingPassword() {

        SoftAssert softAssert = new SoftAssert();
        softAssert.assertTrue(loginForm.isLoginErrorMessagePresent(), "Login error button not found");
        String displayedErrMessage = loginForm.getLoginErrorMessage();
        softAssert.assertTrue(displayedErrMessage.contains("Epic sadface: Password is required"), "<<Epic sadface: Password is required>> text not displayed");
        softAssert.assertTrue(displayedErrMessage.contains(errmessage), "The actual message is: " + displayedErrMessage + ", Expected: " + errmessage);
        softAssert.assertAll();

    }
    
    @Test(groups = {  "missingUserName" })
    public void verifyLoginmissingUserName() {

        SoftAssert softAssert = new SoftAssert();
        softAssert.assertTrue(loginForm.isLoginErrorMessagePresent(), "Login error button not found");
        String displayedErrMessage = loginForm.getLoginErrorMessage();
        softAssert.assertTrue(displayedErrMessage.contains("Epic sadface: Username is required"), "<<Epic sadface: Username is required>> text not displayed");
        softAssert.assertTrue(displayedErrMessage.contains(errmessage), "The actual message is: " + displayedErrMessage + ", Expected: " + errmessage);
        softAssert.assertAll();
   ...
   // @BeforeMethod(alwaysRun = true)
    @BeforeGroups(groups = { "valid", "lockedout", "invalid", "missingPassword","missingUserName" })
